  ...
  return <nav className="fixed top-0 left-0 w-full bg-white/95 backdrop-blur-xl border-b border-slate-200/50 z-50 shadow-sm transition-all duration-300 hover:shadow-md">
      ...
      transitionProperty: "max-height,opacity"
        ...
        <div className={`
          ...
          `}>
          {NAV_ITEMS.map(item => <a key={item} href={`#${item.toLowerCase().replace(" ", "-")}`} className="block w-full text-slate-700 hover:bg-blue-100 hover:text-blue-700 font-medium text-lg tracking-wide transition-all duration-200 rounded-lg py-3 px-4 text-left" onClick={() => setIsMenuOpen(false)}>
              {item}
            </a>)}
          <div className="pt-2">
            <Button size="lg" className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-lg rounded-full font-medium transition-all duration-300 py-4 text-lg">
