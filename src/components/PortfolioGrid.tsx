        ...
        {portfolio.map((project, index) => (
          <AnimatedWrapper key={project.title} animation="zoom-in" delay={`${index * 150}ms`}>
            <article
              ...
              className="group glass-card rounded-[2.5rem] overflow-hidden transition-all duration-700 hover:shadow-[0_32px_64px_rgba(0,0,0,0.12)] hover:-translate-y-3 h-full flex flex-col border border-white/40 shadow-xl"
            >
              {/* Gradient header bar */}
              ...
              <div className="p-10 flex flex-col flex-grow">
                <div className="flex items-start justify-between mb-8">
                  <div className={`p-4 rounded-2xl bg-gradient-to-br ${project.gradient} shadow-xl group-hover:scale-110 transition-transform duration-500`}>
                    <project.icon className="h-7 w-7 text-white" />
                  </div>
                  <span className="text-xs font-bold uppercase tracking-widest text-blue-600 bg-blue-50 px-4 py-2 rounded-full border border-blue-100">
                    {project.tag}
                  </span>
                </div>

                <p className="font-semibold text-xs text-slate-400 uppercase tracking-[0.2em] mb-3">{project.category}</p>
                <h3 className="text-3xl font-bold text-slate-900 mb-4 group-hover:text-blue-600 transition-colors duration-500 leading-tight">
                  {project.title}
                </h3>
                <p className="text-slate-500 text-lg leading-relaxed flex-grow font-light">
                  {project.description}
                </p>

                <div className="mt-8 flex items-center text-blue-600 font-bold text-base opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                  <span className="border-b-2 border-blue-600/30 group-hover:border-blue-600 transition-all">Explore Project</span>
                  <ArrowUpRight className="ml-2 h-5 w-5 animate-bounce-horizontal" />
                </div>
              </div>
            </article>
          </AnimatedWrapper>
