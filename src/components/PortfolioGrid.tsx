  ...
  <section id="work" className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-slate-50 to-blue-50 relative overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto relative z-10">
        ...
        {portfolio.map((project, index) => (
          <AnimatedWrapper key={project.title} animation="zoom-in" delay={`${index * 100}ms`} className="flex w-full sm:basis-[calc(50%-0.75rem)] lg:basis-[calc(33.333%-1rem)]">
            <Card className="bg-white/80 backdrop-blur-sm border-none shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 hover:scale-105 h-full flex flex-col w-full group overflow-hidden relative">
              ...
              <CardHeader className="p-6 sm:p-8 pb-3 flex-grow flex flex-col items-center text-center relative z-10">
                <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 mb-3">
                  <div className={`p-3 rounded-xl ${project.color} group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                    <project.icon className="h-6 w-6 sm:h-7 sm:w-7 text-slate-700 group-hover:text-slate-800 transition-colors duration-300" />
                  </div>
                  <p className="font-medium text-xs sm:text-sm text-slate-500 uppercase tracking-wider group-hover:text-slate-600 transition-colors duration-300 text-center sm:text-left">{project.category}</p>
                </div>
                <CardTitle className="text-lg sm:text-xl font-semibold mb-3 text-slate-800 group-hover:text-slate-900 transition-colors duration-300">{project.title}</CardTitle>
                <CardDescription className="mb-4 text-slate-700 text-sm sm:text-base leading-relaxed group-hover:text-slate-800 transition-colors duration-300">
                  {project.description}
                </CardDescription>
      ...
      <AnimatedWrapper animation="fade-up" delay="300ms" className="text-center mt-16">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 sm:p-8 rounded-2xl shadow-xl text-white">
          <h3 className="text-xl sm:text-2xl font-semibold mb-4">Ready to Start Your Project?</h3>
          <p className="text-base sm:text-lg mb-6 opacity-90">Let's discuss how we can bring your vision to life with cutting-edge technology.</p>
          <button 
            onClick={onContactClick}
            className="bg-white text-blue-600 px-6 sm:px-8 py-3 rounded-full font-semibold hover:bg-gray-100 transition-all duration-300 hover:scale-105 shadow-lg text-sm sm:text-base"
          >
            Get Started Today
          </button>
        </div>
